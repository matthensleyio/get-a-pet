...
public sealed partial class OgFunction(DogRepository dogRepository, IReadOnlyList<ShelterConfig> shelters)
{
    [GeneratedRegex(@"\d+")]
        ...
        CancellationToken ct)
    {
        ...
        string? numericAge = null;
        if (dog.Age != null)
        {
            var match = NumericRegex().Match(dog.Age);
            if (match.Success)
            {
                numericAge = match.Value;
            }
        }

        var descriptionText = numericAge != null
            ? $"They're a {numericAge} year old {dog.Breed ?? "dog"} at {shelterName}"
            : $"They're a {dog.Breed ?? "dog"} at {shelterName}";
